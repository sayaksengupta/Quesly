...
function Lifestyle() {
  ...
  const [lifestyleQueries, setLifestyleQueries] = useState([]);
  ...
  const fetchLifestyleQueries = async () => {
    await axios
      .get("https://quesly-backend.herokuapp.com/get-lifestyle-queries")
      .then((response) => {
        console.log(response);
        setLifestyleQueries(response.data);
      })
      .finally(() => {
        setLoading(false);
      })
  ...
  const CategoryCheck = () => {
    return (
      <p style={{ paddingTop: "1rem", color: "red" }}>Posting in Lifestyle</p>
    );
  };
  ...
  useEffect(() => {
    fetchLifestyleQueries();
  }, [callApi || loadAnswers]);

  console.log(lifestyleQueries);
          ...
          >
            <div id="home-wrapper" class="row">
              ...
              <div id="questionWrapper" class="col-12 col-md-6 col-lg-6">
                ...
                {lifestyleQueries.map((elem, index) => {
                  return (
