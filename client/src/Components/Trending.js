...
function Trending() {
  ...
  const fetchTrendingQueries = async () => {
    await axios
      .get("https://quesly-backend.herokuapp.com/get-trending-queries")
      .then((response) => {
        console.log(response);
        setTrendingQueries(response.data);
      })
      .finally(() => {
        setLoading(false);
      })
  ...
  const CategoryCheck = () => {
    return (
      <p style={{ paddingTop: "1rem", color: "red" }}>Posting in Trending</p>
    );
  };
          ...
          >
            <div id="home-wrapper" class="row">
              ...
              <div id="questionWrapper" class="col-12 col-md-6 col-lg-6">
                ...
                {trendingQueries.map((elem, index) => {
                  return (
