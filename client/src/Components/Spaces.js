...
let user,username;
 ...
 if(location.state.LoggedUser){
  ...
  const memberStatus = (followingUser,index) => {
    ...
    if(!memberFound){
    return <FaPlus className="add-icon"  style={{cursor:"pointer"}} value={followingUser.username} name="members" color="primary" onClick ={ () => addMembers(index)} />
    }else{
      return <FaTrashAlt className="delete-icon" style={{cursor:"pointer"}} value={followingUser.username} name="members" color="primary" onClick ={ () => deleteMembers(index)} />
    }   
  }
    ...
    <>
      {token ? (
        <>
          ...
          >
            <img className="background-spaces" src={bg} width="50%" style={{ objectFit: "contain" }} />
            <Button
                ...
                <div class="modal-body">
                  ...
                  />
                    ...
                    <div className="followed-users">
                      <h5 style={{marginBottom:"1rem"}}>People You Follow :</h5>
                      ...
                      return(
                        <>
                      <div key={followingUser._id} className="users" style={{display: "flex",alignItems:"center",justifyContent:"space-between"}}>
                      <div id="user_img_container" style={{display: "flex",alignItems:"center"}}>
                          <img id="user_img" src={followingUser.profilePic} />
                          <h5 style={{marginBottom:"0", paddingLeft:"1rem"}}>{followingUser.username}</h5>
                      </div>

                      {memberStatus(followingUser,followingUser.username)}
